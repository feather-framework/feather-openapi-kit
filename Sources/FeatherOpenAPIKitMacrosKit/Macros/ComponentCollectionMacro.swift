...
public struct ComponentCollectionMacro: PeerMacro {
    ...
    private static func collectEnumTypes(
        ...
        for member in node.members {
            if let enumDecl = member.decl.as(EnumDeclSyntax.self) {
                ...
                ret += collectEnumTypes(
                    parentNodeName + "." + structDecl.name.text,
                    structDecl.memberBlock
                )
            }
            else if let classDecl = member.decl.as(ClassDeclSyntax.self) {
                ret += collectEnumTypes(
                    parentNodeName + "." + classDecl.name.text,
    ...
    public static func expansion(
        ...
        let collectedMemberTypes = collectEnumTypes(
            groupType,
